...
namespace EnglishLearner
{
    ...
    class Trie
    {
        ...
        private void DFS_Find_All_Instances(string findThisWord, TrieNode whichNode)
        {
            ...
            foreach (string key in nodeKeys)
            {
                ...
                if (Next != null)
                {
                    ...
                    DFS_Find_All_Instances(findThisWord, Next);
        ...
        private void DFS_Append(Phrase currentPhrase, TrieNode whichNode, int iterator = 1)
        {
            ...
            {
                TrieNode Next = null;
                whichNode.Children.TryGetValue(currentPhrase.Phrase_Split_Sentence[iterator], out Next); // TODO: --3-- can possibly eliminate the iterator by just using the NodeDepth. To catch for Root, you'd just have to do an if check that the NodeDepth > 0. If not, add a new root node.
                ...
                else
                {
                    for (int i = iterator; i < currentPhrase.Phrase_Split_Sentence.Length; i++)
                    {
                        ...
                        {
                            whichNode = Next;
                            whichNode.Children.TryGetValue(currentPhrase.Phrase_Split_Sentence[i], out Next);
                            //Next = Current.Next;// TODO: --1-- need to fix this. we don't want a next node in the TrieNode
                        } // while
