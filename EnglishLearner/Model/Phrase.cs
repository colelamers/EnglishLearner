...
namespace EnglishLearner
{
    ...
    public class Phrase
    {
        ...
        private string interpretSubject()
        {
            ...
            var subjectArray = Phrase_Split_Sentence;

            for (int i = 0; i < subjectArray.Length; i++)
            {
                _sql.ExecuteQuery("Select word from entries where word='" + subjectArray[0] + "'");
                ...
                foreach (DataRow n in _sql.ActiveQueryResults.Rows)
                {
                    ...
                    if ((x != null && y != null) && i == 0)
                    {
                        the_subject = (string)x;
                    }
                }
            }
